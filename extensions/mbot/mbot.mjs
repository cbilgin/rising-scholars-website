...
var en$1 = {
	"mbot.entry.name": "mBot",
	"mbot.entry.description": "Drive a Makeblock mBot over Bluetooth"
};
var translations$1 = {
	en: en$1
};
...
var entry = {
  ...
  extensionId: 'mbot',
  extensionURL: 'https://bilgin.github.io/xcx-mbot/dist/mbot.mjs',
  collaborator: 'bilgin',
  iconURL: img$2,
  insetIconURL: img$1,
  get description() {
    return "".concat(formatMessage$1({
      defaultMessage: 'Drive a Makeblock mBot over Bluetooth',
      description: 'Description for this extension',
      id: 'mbot.entry.description'
    }), " (").concat(version, ")");
  },
  tags: ['robotics', 'bluetooth', 'hardware'],
  featured: true,
  disabled: false,
  bluetoothRequired: true,
  internetConnectionRequired: false,
  helpLink: 'https://github.com/bilgin/k5_math',
...
var en = {
	"mbot.name": "mBot",
	"mbot.connect": "connect to mBot",
	"mbot.disconnect": "disconnect mBot",
	"mbot.isConnected": "connected to mBot?",
	"mbot.lastConnectionError": "last mBot error",
	"mbot.move": "move [DIRECTION] at speed [SPEED] for [SECONDS] seconds",
	"mbot.turn": "turn [DIRECTION] at speed [SPEED] for [SECONDS] seconds",
	"mbot.setMotors": "set motors left [LEFT] right [RIGHT]",
	"mbot.stop": "stop mBot",
	"mbot.setLed": "set [LED] LED to red [RED] green [GREEN] blue [BLUE]",
	"mbot.playTone": "play tone [FREQUENCY] Hz for [DURATION] ms",
	"mbot.stopTone": "stop tone",
	"mbot.ultrasonicDistance": "distance (cm) on port [PORT]",
	"mbot.lineFollowerValue": "line follower reading on port [PORT]",
	"mbot.isLineFollowerOnBlack": "[SIDE] line follower on black on port [PORT]?",
	"mbot.isButtonPressed": "onboard button pressed?",
	"mbot.menu.direction.forward": "forward",
	"mbot.menu.direction.backward": "backward",
	"mbot.menu.turn.left": "left",
	"mbot.menu.turn.right": "right",
	"mbot.menu.led.all": "all",
	"mbot.menu.led.left": "left",
	"mbot.menu.led.right": "right"
};
var translations = {
	en: en
};
...
function requireUrlToObj() {
  ...
  function urlToObject(url) {
    return {
      ...
      search: url.search,
      searchParams: _toConsumableArray$1(url.searchParams).map(function (_ref) {
...
function requireBaseLogger() {
  ...
  (function (exports$1) {
    ...
    function createLoggerEnvironment() {
      ...
      var callerIgnorePatterns = usesBrowserStack ? [].concat(_toConsumableArray$1((0, stackTrace_js_1.getDefaultIgnorePatterns)()), [/node_modules[\\/].*tslog/i]) : [].concat(_toConsumableArray$1((0, stackTrace_js_1.getDefaultIgnorePatterns)()), [/node:(?:internal|vm)/i, /\binternal[\\/]/i]);
      var cachedCwd;
      var environment = {
        ...
        prettyFormatErrorObj: function prettyFormatErrorObj(error, settings) {
          ...
          var causeSections = (0, errorUtils_js_1.collectErrorCauses)(error).map(function (cause, index) {
            ...
            return [header].concat(_toConsumableArray$1(frames)).join("\n");
          });
          var placeholderValuesError = {
            errorName: " ".concat(error.name, " "),
            errorMessage: formatErrorMessage(error),
            errorStack: [].concat(_toConsumableArray$1(stackLines), _toConsumableArray$1(causeSections)).join("\n")
          };
        ...
        transportFormatted: function transportFormatted(logMetaMarkup, logArgs, logErrors, logMeta, settings) {
          ...
          if (shouldUseCss(prettyLogs)) {
            ...
            if (hasCssMeta) {
              var _console;
              (_console = console).log.apply(_console, [output].concat(_toConsumableArray$1(cssMeta.styles)));
            } else {
              console.log(output);
            }
            return;
      ...
      function formatWithOptionsSafe(options, args) {
        try {
          return (0, util_inspect_polyfill_js_1.formatWithOptions).apply(void 0, [options].concat(_toConsumableArray$1(args)));
        } catch (_unused) {
    ...
    var BaseLogger$1 = /*#__PURE__*/function () {
      function BaseLogger(settings, logObj) {
        ...
        this.settings = {
          ...
          prefix: _toConsumableArray$1((_settings$prefix = settings === null || settings === void 0 ? void 0 : settings.prefix) !== null && _settings$prefix !== void 0 ? _settings$prefix : []),
          attachedTransports: _toConsumableArray$1((_settings$attachedTra = settings === null || settings === void 0 ? void 0 : settings.attachedTransports) !== null && _settings$attachedTra !== void 0 ? _settings$attachedTra : []),
          overwrite: {
      ...
      return _createClass(BaseLogger, [{
        ...
        value: function log(logLevelId, logLevelName) {
          ...
          var logArgs = [].concat(_toConsumableArray$1(this.settings.prefix), _toConsumableArray$1(resolvedArgs));
        ...
        value: function getSubLogger(settings, logObj) {
          ...
          var subLoggerSettings = _objectSpread$1(_objectSpread$1(_objectSpread$1({}, this.settings), settings), {}, {
            parentNames: ((_this$settings = this.settings) === null || _this$settings === void 0 ? void 0 : _this$settings.parentNames) != null && ((_this$settings2 = this.settings) === null || _this$settings2 === void 0 ? void 0 : _this$settings2.name) != null ? [].concat(_toConsumableArray$1(this.settings.parentNames), [this.settings.name]) : ((_this$settings3 = this.settings) === null || _this$settings3 === void 0 ? void 0 : _this$settings3.name) != null ? [this.settings.name] : undefined,
            prefix: [].concat(_toConsumableArray$1(this.settings.prefix), _toConsumableArray$1((_settings$prefix2 = settings === null || settings === void 0 ? void 0 : settings.prefix) !== null && _settings$prefix2 !== void 0 ? _settings$prefix2 : []))
          });
        ...
        value: function shallowCopy(source) {
          if (Array.isArray(source)) {
            return _toConsumableArray$1(source);
          } else {
            return _objectSpread$1({}, source);
          }
        }
    ...
    var Logger = /*#__PURE__*/function (_BaseLogger_js_1$Base) {
      ...
      return _createClass(Logger, [{
        ...
        value: function log(logLevelId, logLevelName) {
          ...
          return _superPropGet(Logger, "log", this)([logLevelId, logLevelName].concat(args));
        }
      }, {
        key: "silly",
        value: function silly() {
          ...
          return _superPropGet(Logger, "log", this)([0, "SILLY"].concat(args));
        }
      }, {
        key: "trace",
        value: function trace() {
          ...
          return _superPropGet(Logger, "log", this)([1, "TRACE"].concat(args));
        }
      }, {
        key: "debug",
        value: function debug() {
          ...
          return _superPropGet(Logger, "log", this)([2, "DEBUG"].concat(args));
        }
      }, {
        key: "info",
        value: function info() {
          ...
          return _superPropGet(Logger, "log", this)([3, "INFO"].concat(args));
        }
      }, {
        key: "warn",
        value: function warn() {
          ...
          return _superPropGet(Logger, "log", this)([4, "WARN"].concat(args));
        }
      }, {
        key: "error",
        value: function error() {
          ...
          return _superPropGet(Logger, "log", this)([5, "ERROR"].concat(args));
        }
      }, {
        key: "fatal",
        value: function fatal() {
          ...
          return _superPropGet(Logger, "log", this)([6, "FATAL"].concat(args));
        }
      }, {
        key: "getSubLogger",
        value: function getSubLogger(settings, logObj) {
          return _superPropGet(Logger, "getSubLogger", this)([settings, logObj]);
        }
      }]);
...
/**
 * "Makeblock_LE"-advertised module used by the classic mBot: service 0xFFE1,
 * write characteristic 0xFFE3. Same UUIDs code_rover_bot's Flutter app
 * discovers at connect time (`services/mbot_service.dart`), expanded here
 * from the 16-bit alias to the full UUID Web Bluetooth's `requestDevice`
 * expects for a `services` filter.
 *
 * The notify characteristic 0xFFE2 (needed for sensor GET requests —
 * mbotUltrasonicRequestFrame etc. in mbot-protocol.js) was never exercised
 * by the Flutter app, which only ever wrote drive commands. Confirmed
 * instead against an independent open-source Web Bluetooth mBot client
 * (tatiang/mbot-vr's BluetoothLeTransport.ts), which documents the same
 * ffe1 service / ffe2 notify / ffe3 write layout for this exact BLE module —
 * not yet confirmed against Bilgin's own physical mBot.
 */
var MBOT_SERVICE = '0000ffe1-0000-1000-8000-00805f9b34fb';
var MBOT_NOTIFY_CHARACTERISTIC = '0000ffe2-0000-1000-8000-00805f9b34fb';
var MBOT_WRITE_CHARACTERISTIC = '0000ffe3-0000-1000-8000-00805f9b34fb';
var SENSOR_RESPONSE_TIMEOUT_MS = 1000;
var MBOT_DISCOVERY_OPTIONS = {
  filters: [{
    namePrefix: 'Makeblock'
  }],
  optionalServices: [MBOT_SERVICE]
};
...
/**
 * Prefer Web Bluetooth (works in Chrome on the target Android tablet)
 * and only fall back to Scratch Link's BLE class when it's unavailable —
 * same selection Xcratch's own reference extensions use, so a desktop
 * build of this extension keeps working without a code change.
 */
...
// Scratch Link's BLE class relays writes over a JSON-RPC socket, so a raw
// byte array can't cross it as-is — base64 works for both that path and
// WebBLE (which decodes it right back to bytes), so always send this way.
...
var base64ToBytes = function base64ToBytes(base64) {
  return Array.from(atob(base64), function (char) {
    return char.charCodeAt(0);
  });
};

/**
 * Owns the BLE session lifecycle for one mBot connection: scanning,
 * connect/disconnect, and writing pre-built command frames (see
 * mbot-protocol.js). There's no RootProtocol-style packet/CRC layer here —
 * mBot's `0xFF 0x55` frames are simpler and already fully built by the
 * caller, so this class only moves bytes.
 */
var MBotTransport = /*#__PURE__*/function () {
  function MBotTransport(runtime, extensionId) {
    _classCallCheck$1(this, MBotTransport);
    this.runtime = runtime;
    ...
    /**
     * At most one outstanding sensor GET request at a time — {@link
     * request} resolves it from the next notification that arrives, or
     * times out. mBot's firmware answers whatever GET it most recently
     * received, so there's no per-request id to demultiplex several in
     * flight; Scratch reporter blocks call this sequentially anyway.
     * @type {?{resolve: Function, reject: Function, timeout: number}}
     */
    this._pendingRequest = null;

    // Lets Scratch's built-in "Connect a device" dialog find and drive
    // this extension the same way it drives any first-party peripheral.
    this.runtime.registerPeripheralExtension(extensionId, this);
    this.onConnect = this.onConnect.bind(this);
    this.reset = this.reset.bind(this);
    this._registerRuntimeDiagnostics();
  }
  return _createClass$1(MBotTransport, [{
    key: "scan",
    value: function scan() {
      ...
      try {
        this.ble = new BLEAdapter(this.runtime, this.extensionId, MBOT_DISCOVERY_OPTIONS, this.onConnect, this.reset);
      } catch (error) {
        this.setError(error);
        throw error;
      }
    ...
    value: function onConnect() {
      var _this = this;
      // Subscribes so sensor GET requests (mbot-protocol.js's
      // mbotUltrasonicRequestFrame etc.) have somewhere to receive their
      // response. If this particular mBot's BLE module doesn't expose
      // ffe2, this rejects and driving still works fine — only the
      // sensor reporter blocks would time out.
      this.ble.startNotifications(MBOT_SERVICE, MBOT_NOTIFY_CHARACTERISTIC, function (message) {
        _this._handleNotification(base64ToBytes(message));
    ...
    value: function write(bytes) {
      var _this2 = this;
      if (!this.isConnected()) return Promise.reject(new Error('Not connected to an mBot.'));
      return this.ble.write(MBOT_SERVICE, MBOT_WRITE_CHARACTERISTIC, bytesToBase64(bytes), 'base64', false).catch(function (error) {
    ...
    /**
     * Sends a sensor GET request frame and resolves with the raw response
     * bytes from the next BLE notification — the caller (index.js) decodes
     * them with the matching mbot-protocol.js parser. Rejects if nothing
     * arrives within {@link SENSOR_RESPONSE_TIMEOUT_MS}.
     * @param {number[]} bytes - a GET request frame.
     * @returns {Promise<number[]>} - the raw notification payload.
     */
  }, {
    key: "request",
    value: function request(bytes) {
      var _this3 = this;
      if (!this.isConnected()) return Promise.reject(new Error('Not connected to an mBot.'));
      if (this._pendingRequest) this._pendingRequest.reject(new Error('Superseded by a newer request.'));
      return new Promise(function (resolve, reject) {
        var timeout = setTimeout(function () {
          if (_this3._pendingRequest && _this3._pendingRequest.timeout === timeout) {
            _this3._pendingRequest = null;
            reject(new Error('mBot did not respond in time.'));
          }
        }, SENSOR_RESPONSE_TIMEOUT_MS);
        _this3._pendingRequest = {
          resolve: resolve,
          reject: reject,
          timeout: timeout
        };
        _this3.write(bytes).catch(function (error) {
          if (_this3._pendingRequest && _this3._pendingRequest.timeout === timeout) {
    ...
    value: function _handleNotification(bytes) {
      if (!this._pendingRequest) return;
      clearTimeout(this._pendingRequest.timeout);
      var resolve = this._pendingRequest.resolve;
      this._pendingRequest = null;
      resolve(bytes);
    }
  }, {
    ...
    value: function reset() {
      ...
      if (this._pendingRequest) {
        this._pendingRequest.reject(new Error('Disconnected from the mBot.'));
        this._pendingRequest = null;
      }
    }
  }, {
    ...
    value: function setError(error) {
      this.lastError = error && error.message ? error.message : String(error || 'BLE error');
    }
  }, {
    ...
    value: function _registerRuntimeDiagnostics() {
      ...
      this.runtime.on(RuntimeClass.PERIPHERAL_REQUEST_ERROR, function (details) {
        if (!details || !details.extensionId || details.extensionId === _this4.extensionId) {
          _this4.lastError = 'Failed to request a BLE connection.';
        }
      });
      this.runtime.on(RuntimeClass.PERIPHERAL_SCAN_TIMEOUT, function () {
        _this4.lastError = 'No mBot found nearby — make sure it\'s powered on.';
      });
      this.runtime.on(RuntimeClass.PERIPHERAL_CONNECTION_LOST_ERROR, function (details) {
        if (!details || !details.extensionId || details.extensionId === _this4.extensionId) {
          _this4.lastError = 'Lost connection to the mBot.';
        }
      });
    }
  }]);
...
var mbotProtocol;
var hasRequiredMbotProtocol;
function requireMbotProtocol() {
  if (hasRequiredMbotProtocol) return mbotProtocol;
  hasRequiredMbotProtocol = 1;
  var FRAME_START_1 = 0xFF;
  var FRAME_START_2 = 0x55;
  var DEFAULT_IDX = 0x00;
  var ACTION_RUN = 0x02;
  var ACTION_GET = 0x01;
  var DEVICE_ULTRASONIC = 0x01;
  var DEVICE_JOYSTICK = 0x05;
  var DEVICE_RGBLED = 0x08;
  var DEVICE_MOTOR = 0x0A;
  var DEVICE_LINEFOLLOWER = 0x11;
  var DEVICE_TONE = 0x22;
  var DEVICE_BUTTON_INNER = 0x23;

  /**
   * Single-motor port ids for {@link mbotMotorFrame} (used by the calibration
   * screen to test one wheel at a time). {@link mbotMoveFrame} below is what
   * normal driving uses, since it sets both motors in one frame.
   */
  var PORT_M1 = 0x09;
  var PORT_M2 = 0x0A;

  /**
   * The onboard RGB LEDs and onboard pushbutton aren't wired to a numbered
   * expansion port — the firmware still expects frame bytes as if they were,
   * using fixed values ("led on board" = port 7 / slot 2 in mBot.s2e; the
   * onboard button's port argument of 7 indexes `analogs[7]` = pin A7).
   */
  var ONBOARD_PORT = 0x07;
  var ONBOARD_LED_SLOT = 0x02;
  var RGB_LED_ALL = 0x00;
  var RGB_LED_RIGHT = 0x01;
  var RGB_LED_LEFT = 0x02;
  var MAX_MOTOR_SPEED = 255;
  var clampSpeed = function clampSpeed(speed) {
    if (speed > MAX_MOTOR_SPEED) return MAX_MOTOR_SPEED;
    if (speed < -MAX_MOTOR_SPEED) return -MAX_MOTOR_SPEED;
    return speed;
  };
  var clampByte = function clampByte(value) {
    return Math.max(0, Math.min(255, Math.round(value)));
  };

  /**
   * Little-endian signed 16-bit encoding, matching the firmware's byte union
   * and python-for-mbot's `struct.pack("h", ...)` (native byte order, i.e.
   * little-endian on the boards these targets).
   * @param {number} value - signed value already clamped to fit in 16 bits.
   * @returns {number[]} - two bytes, low byte first.
   */
  var int16LE = function int16LE(value) {
    var bits = value & 0xFFFF;
    return [bits & 0xFF, bits >> 8 & 0xFF];
  };

  /**
   * Assembles a full frame from its body (everything after the length byte)
   * so the length byte itself is always derived, never hand-counted —
   * hand-counting it separately is exactly the kind of off-by-one that stays
   * silently wrong until it's debugged live over BLE.
   * @param {number[]} body - idx, action, device, then device-specific bytes.
   * @returns {number[]} - the complete frame including the 0xFF 0x55 header.
   */
  var buildFrame = function buildFrame(body) {
    return [FRAME_START_1, FRAME_START_2, body.length].concat(_toConsumableArray(body));
  };

  /**
   * Drives both wheel motors in a single frame (device JOYSTICK, 0x05) so
   * they start together — matters for open-loop timing, where even a small
   * gap between two separate single-motor commands would make the rover
   * veer. leftSpeed/rightSpeed are each clamped to +/-255; positive drives
   * that wheel forward.
   * @param {number} leftSpeed - left wheel speed, -255 to 255.
   * @param {number} rightSpeed - right wheel speed, -255 to 255.
   * @returns {number[]} - the command frame bytes.
   */
  var mbotMoveFrame = function mbotMoveFrame(leftSpeed, rightSpeed) {
    var left = clampSpeed(leftSpeed);
    var right = clampSpeed(rightSpeed);
    return buildFrame([DEFAULT_IDX, ACTION_RUN, DEVICE_JOYSTICK].concat(_toConsumableArray(int16LE(-left)), _toConsumableArray(int16LE(right))));
  };
  var mbotStopFrame = function mbotStopFrame() {
    return mbotMoveFrame(0, 0);
  };

  /**
   * Single-motor command (device MOTOR, 0x0A) for PORT_M1/PORT_M2 — only
   * used by calibration/testing one wheel at a time; normal driving always
   * goes through mbotMoveFrame.
   * @param {number} port - PORT_M1 or PORT_M2.
   * @param {number} speed - motor speed, -255 to 255.
   * @returns {number[]} - the command frame bytes.
   */
  var mbotMotorFrame = function mbotMotorFrame(port, speed) {
    return buildFrame([DEFAULT_IDX, ACTION_RUN, DEVICE_MOTOR, port].concat(_toConsumableArray(int16LE(clampSpeed(speed)))));
  };

  /**
   * Sets the onboard RGB LEDs (device RGBLED, 0x08). `led` selects which
   * physical LED: RGB_LED_ALL/RGB_LED_LEFT/RGB_LED_RIGHT.
   * NOT yet verified against real hardware or a second source — only
   * against `mbot_firmware.ino`'s `runModule()` RGBLED case.
   * @param {number} led - RGB_LED_ALL, RGB_LED_LEFT, or RGB_LED_RIGHT.
   * @param {number} red - 0-255.
   * @param {number} green - 0-255.
   * @param {number} blue - 0-255.
   * @returns {number[]} - the command frame bytes.
   */
  var mbotRgbLedFrame = function mbotRgbLedFrame(led, red, green, blue) {
    return buildFrame([DEFAULT_IDX, ACTION_RUN, DEVICE_RGBLED, ONBOARD_PORT, ONBOARD_LED_SLOT, led, clampByte(red), clampByte(green), clampByte(blue)]);
  };

  /**
   * Plays a tone on the onboard buzzer (device TONE, 0x22). A frequency of 0
   * stops any currently playing tone, matching the firmware's `noTone()`
   * branch. NOT yet verified against real hardware or a second source — only
   * against `mbot_firmware.ino`'s `runModule()` TONE case.
   * @param {number} frequencyHz - tone frequency in Hz, or 0 to stop.
   * @param {number} durationMs - how long the firmware should sound the tone.
   * @returns {number[]} - the command frame bytes.
   */
  var mbotToneFrame = function mbotToneFrame(frequencyHz, durationMs) {
    return buildFrame([DEFAULT_IDX, ACTION_RUN, DEVICE_TONE].concat(_toConsumableArray(int16LE(Math.max(0, Math.round(frequencyHz)))), _toConsumableArray(int16LE(Math.max(0, Math.round(durationMs))))));
  };
  var mbotToneOffFrame = function mbotToneOffFrame() {
    return mbotToneFrame(0, 0);
  };

  /**
   * Requests an ultrasonic distance reading (device ULTRASONIC_SENSOR, 0x01).
   * Response is a headered float — see {@link parseFloatResponse}'s doc for
   * why that's true despite the reference firmware source suggesting
   * otherwise for this specific device.
   * @param {number} port - the port the ultrasonic sensor is plugged into.
   * @returns {number[]} - the request frame bytes.
   */
  var mbotUltrasonicRequestFrame = function mbotUltrasonicRequestFrame(port) {
    return buildFrame([DEFAULT_IDX, ACTION_GET, DEVICE_ULTRASONIC, port]);
  };

  /**
   * Requests a line-follower reading (device LINEFOLLOWER, 0x11).
   * Response is a headered float — see {@link parseFloatResponse}.
   * @param {number} port - the port the line-follower sensor is plugged into.
   * @returns {number[]} - the request frame bytes.
   */
  var mbotLineFollowerRequestFrame = function mbotLineFollowerRequestFrame(port) {
    return buildFrame([DEFAULT_IDX, ACTION_GET, DEVICE_LINEFOLLOWER, port]);
  };

  /**
   * Requests the onboard pushbutton's state (device BUTTON_INNER, 0x23).
   * The firmware's actual semantics are an XOR, `s ^ currentPressed`, of the
   * state byte you send against its live reading — sending `s=0` (asking
   * about "pressed") makes that just `currentPressed` directly: 1 = pressed,
   * 0 = released (see {@link parseButtonPressedResponse}; confirmed live —
   * an earlier version of this had the comparison backwards). Port is fixed
   * at 7, which the firmware looks up in its `analogs[]` table as pin A7 —
   * the classic mBot's onboard-button pin.
   * @returns {number[]} - the request frame bytes.
   */
  var mbotButtonRequestFrame = function mbotButtonRequestFrame() {
    return buildFrame([DEFAULT_IDX, ACTION_GET, DEVICE_BUTTON_INNER, ONBOARD_PORT, 0x00]);
  };

  /**
   * Decodes a firmware float32 response value: 1 tag byte (2 = float) then
   * 4 little-endian IEEE-754 bytes, matching `sendFloat()` in the firmware.
   * @param {number[]} bytes - exactly the tag byte followed by 4 value bytes.
   * @returns {number} - the decoded float.
   */
  var decodeFloatValue = function decodeFloatValue(bytes) {
    var view = new DataView(new Uint8Array(bytes.slice(1, 5)).buffer);
    return view.getFloat32(0, true);
  };

  /**
   * Parses a headered float response (`0xFF 0x55 <idx>` then the tagged
   * value) — used for both the ultrasonic sensor and the line-follower.
   *
   * `mbot_firmware.ino`'s source has a special case skipping this header for
   * the ultrasonic sensor specifically (`if (device != ULTRASONIC_SENSOR)`
   * around `writeHead()` in `parseData()`) — that does NOT hold on the actual
   * firmware Bilgin's mBot runs, confirmed live: a real distance request
   * decoded as garbage (~1.8e-40) until the header was accounted for here
   * too. Trusting the hardware over that firmware snapshot, which is
   * apparently stale or from a different revision.
   * @param {number[]} bytes - the raw notification payload.
   * @returns {number} - the decoded float value.
   */
  var parseFloatResponse = function parseFloatResponse(bytes) {
    return decodeFloatValue(bytes.slice(3));
  };

  /**
   * Parses the onboard button's response to {@link mbotButtonRequestFrame}:
   * 1 = pressed, 0 = released.
   * @param {number[]} bytes - the raw notification payload, `0xFF 0x55 <idx>` then a 1-byte value.
   * @returns {boolean} - true if the button is currently pressed.
   */
  var parseButtonPressedResponse = function parseButtonPressedResponse(bytes) {
    return bytes[4] === 1;
  };

  /**
   * Corrects the ultrasonic sensor's raw `distanceCm()` reading, which runs
   * consistently ~20-25% low on this specific mBot. Fitted (least squares)
   * from three live tape-measure readings against the raw sensor value:
   * (10cm, 8.39), (15cm, 12.3), (26cm, 20.8) — residuals under 0.02cm across
   * that whole range, so this is a real per-unit calibration constant, not
   * noise. It's specific to this physical sensor/board, not a general mBot
   * property — a different unit could need a different constant.
   * @param {number} rawCm - the value decoded from the firmware's response.
   * @returns {number} - the corrected distance in centimeters.
   */
  var calibrateUltrasonicCm = function calibrateUltrasonicCm(rawCm) {
    return 1.2901 * rawCm - 0.8417;
  };

  /**
   * Bit masks for {@link isLineFollowerSideOnBlack} against the raw value
   * from {@link mbotLineFollowerRequestFrame}'s response (`dRead1()*2 +
   * dRead2()` per the firmware). These weights match mBot.s2e's own
   * "leftRight" menu constants (leftSide=2, rightSide=1) exactly, and were
   * cross-checked live: a light surface read 3 (both bits set, both sensors
   * HIGH) and a dark surface read 0 (both LOW) — so HIGH/1 = light,
   * LOW/0 = dark, for each sensor independently.
   */
  var LINE_FOLLOWER_LEFT_MASK = 2;
  var LINE_FOLLOWER_RIGHT_MASK = 1;

  /**
   * @param {number} rawValue - the decoded line-follower value (0-3).
   * @param {number} mask - LINE_FOLLOWER_LEFT_MASK or LINE_FOLLOWER_RIGHT_MASK.
   * @returns {boolean} - true if that sensor currently reads a dark surface.
   */
  var isLineFollowerSideOnBlack = function isLineFollowerSideOnBlack(rawValue, mask) {
  ...
  mbotProtocol = {
    ...
    RGB_LED_ALL: RGB_LED_ALL,
    RGB_LED_LEFT: RGB_LED_LEFT,
    RGB_LED_RIGHT: RGB_LED_RIGHT,
    mbotMoveFrame: mbotMoveFrame,
    mbotStopFrame: mbotStopFrame,
    mbotMotorFrame: mbotMotorFrame,
    mbotRgbLedFrame: mbotRgbLedFrame,
    mbotToneFrame: mbotToneFrame,
    mbotToneOffFrame: mbotToneOffFrame,
    mbotUltrasonicRequestFrame: mbotUltrasonicRequestFrame,
    mbotLineFollowerRequestFrame: mbotLineFollowerRequestFrame,
    mbotButtonRequestFrame: mbotButtonRequestFrame,
    parseFloatResponse: parseFloatResponse,
    calibrateUltrasonicCm: calibrateUltrasonicCm,
    LINE_FOLLOWER_LEFT_MASK: LINE_FOLLOWER_LEFT_MASK,
    LINE_FOLLOWER_RIGHT_MASK: LINE_FOLLOWER_RIGHT_MASK,
    isLineFollowerSideOnBlack: isLineFollowerSideOnBlack,
    parseButtonPressedResponse: parseButtonPressedResponse
  };
  return mbotProtocol;
}

var mbotProtocolExports = requireMbotProtocol();
 ...
 * @type {string}
 */
var extensionURL = 'https://bilgin.github.io/xcx-mbot/dist/mbot.mjs';

/**
 * Default wheel speed (0-255) for the timed move/turn blocks, matching the
 * speed code_rover_bot's Flutter app drives at.
 */
var DEFAULT_SPEED = 150;

/**
 * mBot.s2e's own defaults for these ports (Port3 for the ultrasonic sensor,
 * Port2 for the line follower) — kept as this extension's defaults too,
 * since that's where Makeblock's standard mBot kit wiring puts them.
 */
var DEFAULT_ULTRASONIC_PORT = 3;
var DEFAULT_LINE_FOLLOWER_PORT = 2;
var LED_TARGET_TO_CONSTANT = {
  all: mbotProtocolExports.RGB_LED_ALL,
  left: mbotProtocolExports.RGB_LED_LEFT,
  right: mbotProtocolExports.RGB_LED_RIGHT
};
var SIDE_TO_LINE_FOLLOWER_MASK = {
  left: mbotProtocolExports.LINE_FOLLOWER_LEFT_MASK,
  right: mbotProtocolExports.LINE_FOLLOWER_RIGHT_MASK
};

/**
 * Scratch 3.0 blocks to drive a Makeblock mBot over Bluetooth LE.
 */
var ExtensionBlocks = /*#__PURE__*/function () {
  /**
   * Construct a set of blocks to drive an mBot.
  ...
  function ExtensionBlocks(runtime) {
    ...
    /**
     * Owns the BLE connection to the mBot.
     * @type {MBotTransport}
     */
    this.transport = new MBotTransport(runtime, ExtensionBlocks.EXTENSION_ID);
  }
  ...
  return _createClass$1(ExtensionBlocks, [{
    ...
    value: function getInfo() {
      ...
      return {
        ...
        blocks: [{
          ...
          text: formatMessage({
            id: 'mbot.connect',
            default: 'connect to mBot',
            description: 'command to start scanning for an mBot'
          }),
          func: 'connect'
        }, {
          opcode: 'disconnect',
          ...
          text: formatMessage({
            id: 'mbot.disconnect',
            default: 'disconnect mBot',
            description: 'command to disconnect from the mBot'
          }),
          func: 'disconnect'
        }, {
          opcode: 'isConnected',
          ...
          text: formatMessage({
            id: 'mbot.isConnected',
            default: 'connected to mBot?',
            description: 'true if connected to an mBot'
          }),
          func: 'isConnected'
        }, {
          ...
          text: formatMessage({
            id: 'mbot.lastConnectionError',
            default: 'last mBot error',
            description: 'the most recent BLE connection error message'
          }),
          func: 'lastConnectionError'
        }, '---', {
          opcode: 'move',
          blockType: BlockType.COMMAND,
          text: formatMessage({
            id: 'mbot.move',
            default: 'move [DIRECTION] at speed [SPEED] for [SECONDS] seconds',
            description: 'command to drive both wheels for a duration'
          }),
          func: 'move',
          arguments: {
            DIRECTION: {
          ...
          text: formatMessage({
            id: 'mbot.turn',
            default: 'turn [DIRECTION] at speed [SPEED] for [SECONDS] seconds',
            description: 'command to spin in place for a duration'
          }),
          func: 'turn',
          arguments: {
            DIRECTION: {
          ...
          text: formatMessage({
            ...
            description: 'command to set raw wheel speeds without auto-stopping'
          }),
          func: 'setMotors',
          arguments: {
            LEFT: {
          ...
          text: formatMessage({
            id: 'mbot.stop',
            default: 'stop mBot',
            description: 'command to stop both wheels'
          }),
          func: 'stop'
        }, '---', {
          opcode: 'setLed',
          ...
          text: formatMessage({
            id: 'mbot.setLed',
            default: 'set [LED] LED to red [RED] green [GREEN] blue [BLUE]',
            description: 'command to set the onboard RGB LED color'
          }),
          func: 'setLed',
          arguments: {
            LED: {
          ...
          text: formatMessage({
            id: 'mbot.playTone',
            default: 'play tone [FREQUENCY] Hz for [DURATION] ms',
            description: 'command to play a tone on the onboard buzzer'
          }),
          func: 'playTone',
          arguments: {
            FREQUENCY: {
          ...
          text: formatMessage({
            id: 'mbot.stopTone',
            default: 'stop tone',
            description: 'command to stop the onboard buzzer'
          }),
          func: 'stopTone'
        }, '---', {
          ...
          blockType: BlockType.REPORTER,
          text: formatMessage({
            id: 'mbot.ultrasonicDistance',
            default: 'distance (cm) on port [PORT]',
            description: 'reporter for the ultrasonic sensor distance reading'
          }),
          func: 'ultrasonicDistance',
          arguments: {
            PORT: {
              type: ArgumentType.NUMBER,
              defaultValue: DEFAULT_ULTRASONIC_PORT
            }
          }
        }, {
          opcode: 'lineFollowerValue',
          blockType: BlockType.REPORTER,
          text: formatMessage({
            id: 'mbot.lineFollowerValue',
            default: 'line follower reading on port [PORT]',
            description: 'reporter for the raw line follower sensor reading'
          }),
          func: 'lineFollowerValue',
          arguments: {
            PORT: {
              type: ArgumentType.NUMBER,
              defaultValue: DEFAULT_LINE_FOLLOWER_PORT
            }
          }
        }, {
          opcode: 'isLineFollowerOnBlack',
          blockType: BlockType.BOOLEAN,
          text: formatMessage({
            id: 'mbot.isLineFollowerOnBlack',
            default: '[SIDE] line follower on black on port [PORT]?',
            description: 'boolean reporter for one line follower sensor'
          }),
          func: 'isLineFollowerOnBlack',
          arguments: {
            SIDE: {
              type: ArgumentType.STRING,
              menu: 'turnMenu',
          ...
          blockType: BlockType.BOOLEAN,
          text: formatMessage({
            id: 'mbot.isButtonPressed',
            default: 'onboard button pressed?',
            description: 'boolean reporter for the onboard pushbutton'
          }),
        ...
        menus: {
          directionMenu: {
            ...
            items: [{
              text: formatMessage({
                id: 'mbot.menu.direction.forward',
                default: 'forward',
              ...
              text: formatMessage({
                id: 'mbot.menu.direction.backward',
                default: 'backward',
          ...
          turnMenu: {
            ...
            items: [{
              text: formatMessage({
                id: 'mbot.menu.turn.left',
                default: 'left',
                description: 'label for turning left'
              }),
              value: 'left'
            }, {
              text: formatMessage({
                id: 'mbot.menu.turn.right',
                default: 'right',
                description: 'label for turning right'
              }),
              value: 'right'
            }]
          },
          ledMenu: {
            ...
            items: [{
              text: formatMessage({
                id: 'mbot.menu.led.all',
                default: 'all',
                description: 'label for both onboard LEDs'
              }),
              value: 'all'
            }, {
              text: formatMessage({
                id: 'mbot.menu.led.left',
                default: 'left',
                description: 'label for the left onboard LED'
              }),
              value: 'left'
            }, {
              text: formatMessage({
                id: 'mbot.menu.led.right',
                default: 'right',
                description: 'label for the right onboard LED'
              }),
              value: 'right'
            }]
          }
    ...
    value: function move(args) {
      ...
      var signedSpeed = args.DIRECTION === 'backward' ? -speed : speed;
      return this._driveTimed(signedSpeed, signedSpeed, args.SECONDS);
    }
  }, {
    key: "turn",
    value: function turn(args) {
      var speed = Cast.toNumber(args.SPEED);
      var turningRight = args.DIRECTION === 'right';
      var left = turningRight ? speed : -speed;
      var right = turningRight ? -speed : speed;
      return this._driveTimed(left, right, args.SECONDS);
    }
  }, {
    key: "setMotors",
    value: function setMotors(args) {
      this._send(mbotProtocolExports.mbotMoveFrame(Cast.toNumber(args.LEFT), Cast.toNumber(args.RIGHT)));
    }
  }, {
    key: "stop",
    value: function stop() {
      this._send(mbotProtocolExports.mbotStopFrame());
    }
  }, {
    key: "setLed",
    value: function setLed(args) {
      var _LED_TARGET_TO_CONSTA;
      var led = (_LED_TARGET_TO_CONSTA = LED_TARGET_TO_CONSTANT[args.LED]) !== null && _LED_TARGET_TO_CONSTA !== void 0 ? _LED_TARGET_TO_CONSTA : mbotProtocolExports.RGB_LED_ALL;
      this._send(mbotProtocolExports.mbotRgbLedFrame(led, Cast.toNumber(args.RED), Cast.toNumber(args.GREEN), Cast.toNumber(args.BLUE)));
    }
  }, {
    key: "playTone",
    value: function playTone(args) {
      this._send(mbotProtocolExports.mbotToneFrame(Cast.toNumber(args.FREQUENCY), Cast.toNumber(args.DURATION)));
    }
  }, {
    key: "stopTone",
    value: function stopTone() {
      this._send(mbotProtocolExports.mbotToneOffFrame());
    }
  }, {
    key: "ultrasonicDistance",
    value: function ultrasonicDistance(args) {
      return this._request(mbotProtocolExports.mbotUltrasonicRequestFrame(Cast.toNumber(args.PORT)), function (bytes) {
        return mbotProtocolExports.calibrateUltrasonicCm(mbotProtocolExports.parseFloatResponse(bytes));
      }, 0);
    }
  }, {
    key: "lineFollowerValue",
    value: function lineFollowerValue(args) {
      return this._request(mbotProtocolExports.mbotLineFollowerRequestFrame(Cast.toNumber(args.PORT)), mbotProtocolExports.parseFloatResponse, 0);
    }
  }, {
    key: "isLineFollowerOnBlack",
    value: function isLineFollowerOnBlack(args) {
      var _SIDE_TO_LINE_FOLLOWE;
      var mask = (_SIDE_TO_LINE_FOLLOWE = SIDE_TO_LINE_FOLLOWER_MASK[args.SIDE]) !== null && _SIDE_TO_LINE_FOLLOWE !== void 0 ? _SIDE_TO_LINE_FOLLOWE : mbotProtocolExports.LINE_FOLLOWER_LEFT_MASK;
      return this._request(mbotProtocolExports.mbotLineFollowerRequestFrame(Cast.toNumber(args.PORT)), function (bytes) {
        return mbotProtocolExports.isLineFollowerSideOnBlack(mbotProtocolExports.parseFloatResponse(bytes), mask);
      }, false);
    }
  }, {
    key: "isButtonPressed",
    value: function isButtonPressed() {
      return this._request(mbotProtocolExports.mbotButtonRequestFrame(), mbotProtocolExports.parseButtonPressedResponse, false);
    }

    /**
     * Drives both wheels, then auto-stops after the given duration. The mBot's
     * classic firmware has no motion-completion feedback (unlike e.g. iRobot
     * Root), so open-loop timing is the only option — the block itself waits
     * out the duration before resolving, so sequential drive blocks in a
     * script run one after another rather than overlapping.
     * @param {number} leftSpeed - left wheel speed, -255 to 255.
     * @param {number} rightSpeed - right wheel speed, -255 to 255.
     * @param {number} seconds - how long to drive before stopping.
     * @returns {Promise} - resolves once the stop command has been sent.
     */
  }, {
    key: "_driveTimed",
    value: function _driveTimed(leftSpeed, rightSpeed, seconds) {
      var _this = this;
      var durationMs = Math.max(0, Cast.toNumber(seconds)) * 1000;
      this._send(mbotProtocolExports.mbotMoveFrame(leftSpeed, rightSpeed));
      return new Promise(function (resolve) {
        setTimeout(function () {
          _this._send(mbotProtocolExports.mbotStopFrame());
          resolve();
        }, durationMs);
      });
    }
    ...
    /**
     * Fire-and-forget write: a stalled BLE write promise (Scratch Link/Scrub
     * can leave one pending after the peripheral already accepted the bytes)
     * shouldn't hang the calling block's script forever.
     * @param {number[]} frame - the command frame bytes.
     */
  }, {
    key: "_send",
    value: function _send(frame) {
      var _this2 = this;
      try {
        var pendingWrite = this.transport.write(frame);
    ...
    /**
     * Sends a sensor GET request and decodes its response, falling back to
     * a default value (rather than leaving a Scratch reporter block blank
     * or throwing mid-script) if the mBot doesn't answer in time — see
     * MBotTransport#request in mbot-ble.js for the timeout itself.
     * @param {number[]} frame - a GET request frame from mbot-protocol.js.
     * @param {function(number[]): *} parseResponse - the matching parser.
     * @param {*} fallbackValue - value to report if the request fails.
     * @returns {Promise<*>} - the parsed sensor value, or fallbackValue.
     */
  }, {
    key: "_request",
    value: function _request(frame, parseResponse, fallbackValue) {
      var _this3 = this;
      return this.transport.request(frame).then(function (bytes) {
